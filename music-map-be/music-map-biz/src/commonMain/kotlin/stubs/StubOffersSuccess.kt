...
fun ICorChainDsl<MusicContext>.stubOffersSuccess(title: String, corSettings: MusicCorSettings) = worker {
    this.title = title
    this.description = """
    ...
    handle {
        logger.doWithLogging(id = this.requestId.toString(), LogLevel.DEBUG) {
            ...
            cardResponse = CardStub.prepareResult {
                cardRequest.id.takeIf { it != MusicCardId.NONE }?.also { this.id = it }
            }
            cardsResponse.addAll(CardStub.prepareOffersList())
        }
    }
}
